...
class QSpyHandle:
    def start_qspy(self, targetPort : str, targetBaud, hostSocket = 6601):
            ...
            cmd = 'qspy -c {} -b {} -u {}'.format(targetPort, str(targetBaud), str(hostSocket))    
            #self.qspy_process = run(cmd, creationflags=CREATE_NEW_CONSOLE, stdin=PIPE)
...
class QutestHandle:
    def start_qutest(self, test_dir, test_name, qspySocket:int):
        ...
        fileListString ='scripts/test_uart_sensor/test_assert.py'
        ...
        #cmd = ['python', qutest_path, fileListString, '""', "localhost:{}".format(qspySocket)]
        print(cmd)
        self.qutest_process = Popen(cmd, creationflags=CREATE_NEW_CONSOLE, stdin=PIPE, stdout=PIPE, stderr=PIPE, universal_newlines=True)
        #sleep(0.1)
...
myQspy = QSpyHandle()
myQutest = QutestHandle()
socketPort = 6601

if __name__ == "__main__":
    myQspy.start_qspy("COM3", 921600, socketPort)
    ...
    #myQutest.start_qutest(test_dir, test_name, socketPort)

    while True:
        stdLine, stdErr = myQspy.qspy_process.communicate()
        if stdLine is not None:
            click.echo(stdLine)
        sleep(0.01)
