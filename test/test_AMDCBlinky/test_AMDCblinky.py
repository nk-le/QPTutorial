...
def on_reset():
    ...
    expect("@timestamp TE0-Arm  Obj=AMDCBlinky::inst.m_timeEvt,AO=AMDCBlinky::inst,Tim=500,Int=500") 
...
expect("@timestamp Query-TE Obj=AMDCBlinky::inst.m_timeEvt,Rate=0,Sig=TIMEOUT_SIG,Tim=500,Int=500,Flags=0x80")
...
expect("@timestamp AO-Post  Sdr=QS_RX,Obj=AMDCBlinky::inst,Evt<Sig=ADJUST_FREQ_SIG,Pool=1,Ref=*>,Que<Free=100,Min=100>")
...
expect("@timestamp TE0-Dis  Obj=AMDCBlinky::inst.m_timeEvt,AO=AMDCBlinky::inst,Tim=500,Int=500")
expect("@timestamp TE0-Arm  Obj=AMDCBlinky::inst.m_timeEvt,AO=AMDCBlinky::inst,Tim=200,Int=200")
...
test("Singularity is when LED freq = 0? Blackhole?")
glb_filter(GRP_AO, 100, 101, 102, 103)  
current_obj(OBJ_SM_AO,"AMDCBlinky::inst")               
current_obj(OBJ_TE,"AMDCBlinky::inst.m_timeEvt")
query_curr(OBJ_SM)      # Query the state machine
